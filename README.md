...
- **右侧工具设置栏**（仿 PixPin）：切换工具显示对应参数页——矩形/圆形的描边+填充+
  填充透明度，文字的字体/字号/粗体/斜体/描边/背景，序号大小与序号值（未选中时为
  下一个序号的起始值），橡皮擦大小，马赛克块大小；
...
- 选择模式：拖动移动标注、**选框手柄调整大小**（单选显示 8 手柄选框，直线/箭头/画笔按
  包围盒缩放，文字调字号，序号调半径）、**选框下方旋转按钮拖动旋转**（Shift 吸附 15°
  步进，马赛克除外）、`Delete` 删除选中、双击文字重新编辑、改色/改线宽作用于选中标注
- **即选即调**：拖画添加形状后自动选中（显示选框、侧栏临时显示其属性），可立即
  调参/移动/缩放；点击同类型已有标注同样即选，点空白恢复新增，无需来回切换选择工具；
  软选中时的编辑只作用于该标注，不影响新标注的默认值
